...
class DraftControllerConfig:
    ...
    acceptance_low: float = 0.30
    acceptance_disable: float = 0.10
    cpu_high: float = 0.85
    ...
    depth_change_cooldown: int = 10
    latency_spike_ms: float = 150.0
    bad_steps_to_disable: int = 15
    oscillation_window: int = 15
...
class AdaptiveDraftController:
    ...
    def step(self, snap: Optional[InferenceSnapshot] = None,
        ...
        # FIX: Added warmup grace period (>10 steps) to prevent false spikes during init
        if snap.latency_ms > self.cfg.latency_spike_ms and self._can_change() and self.monitor._total_steps > 10:
            new_depth = max(self.cfg.min_depth, self._depth - 1)
